...
def addToList(self):
    ...
    var_lname = var_lname.strip() # This will ensure that the first character in each word is capitalizes
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): # enforces the user to provide both names
        conn = sqlite3.connect('db_phonebook.db')
        with conn:
            cursor = conn.cursor()
            ...
            if chkName == 0: # If this is 0 then there is no existance of the fullname and we can add new data
                ...
                cursor.execute("""INSERT INTO tbl_phonebook (col_fname, col_lname, col_fullname, col_phone, col_email) ALUES (?,?,?,?,?)""",(var_fname, var_lname, var_fullname,var_phone, var_email))
...
def onUpdate(self):
    ...
    var_email = self.txt_email.get().strip()
    if (len(var_phone) > 0) and (len(var_email) > 0): # Ensures that there is data present.
        conn = sqlite3.connect('db_phonebook.db')
        with conn:
            cur = conn.cursor()
            ...
            if count == 0 or count2 == 0: # If proposed changes are not already in the database, then proceed
                ...
                if response:
                    ...
                    with conn:
                        cursor = conn.cursor()
                        cursor.execute("""UPDATE tbl_phonebook SET col_phone = '{}', col_email = '{}' WHERE col_fullname = '{2}'""".format(var_phone, var_email, var_value))
                        onClear(self)
                        conn.commit()
                else:
